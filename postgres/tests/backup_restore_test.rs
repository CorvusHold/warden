    ...
    Fut: Future<Output = Result<(), Box<dyn std::error::Error>>>,
{
    ...
    let result = test(config).await;

    // Ensure the container stays alive for the duration of the test
    let final_result = result;

    // Drop container explicitly after test completion
    drop(container);
...
async fn test_full_backup_and_restore() -> Result<(), Box<dyn std::error::Error>> {
    run_with_test_database(|config| async move {
...
async fn test_backup_catalog() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_with_test_database(|config| async move {
        ...
        let mut manager = PostgresManager::new(config.clone(), backup_dir.path().to_path_buf())?;
        ...
        let manager2 = PostgresManager::new(config, backup_dir.path().to_path_buf())?;
